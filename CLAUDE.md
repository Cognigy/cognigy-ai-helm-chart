...
- **Insights Postgres provider**: use the `cognigy-ai.insights.postgresql.*` helpers (`_insights.tpl`) to gate and configure Insights Postgres consumers. Do NOT gate new templates on `$.Values.pgoperator.enabled` directly — that only controls the operator subchart, not which provider consumers use. Use `include "cognigy-ai.insights.postgresql.enabled" $` (returns "true" when provider is operator or external) and `include "cognigy-ai.insights.postgresql.host/port/sslmode/secret" $` for connection values.
